...
import { WebGLTerminalAdapter } from '../rendering/WebGLTerminalAdapter'
import {
...
export class XTerminalFactory {
  ...
  static async createTerminal(
    ...
    // Create terminal manager and open terminal in DOM FIRST
    const manager = new XTermManager(terminalOptions)
    const terminal = await manager.initialize(container)
    // Terminal is now opened in the DOM via manager.initialize()

    // NOW create addon manager and load addons after terminal is in DOM
    ...
    let webglRenderer: WebGLTerminalAdapter | undefined
    ...
    // Setup WebGL renderer if requested
    if (options.webgl?.enabled) {
      // Create a canvas element for WebGL rendering
      ...
      // Add canvas to terminal container
      if (terminal.element) {
        terminal.element.appendChild(canvas)

        webglRenderer = new WebGLTerminalAdapter()
        ...
        try {
          ...
          // Set up buffer update listener to bridge xterm buffer to WebGL
          terminal.onData(() => {
            // Extract terminal buffer content
  ...
  static async createDevelopmentTerminal(
    ...
    return this.createTerminal(container, {
      ...
      webgl: {
        enabled: true,
        enablePerformanceMonitoring: true,
        preserveDrawingBuffer: true, // For debugging screenshots
        debug: true,
      },
      backpressure: {
        enabled: true,
