...
describe('TerminalResizeManager', () => {
  ...
  describe('fit', () => {
    ...
    it('should successfully resize terminal with manual dimension calculation', async () => {
      // Context7 Best Practice: Test successful manual fit path (line 248)
      // This specifically tests the branch where calculateDimensions returns valid dimensions
      vi.doMock('@xterm/addon-fit', () => {
        throw new Error('Import failed')
      })

      const manualManager = new TerminalResizeManager({ debug: true })
      await manualManager.initialize(
        mockTerminal as unknown as Terminal,
        mockContainer
      )
      ...
      // Reset resized flag
      mockTerminal.resized = false

      // Call fit() which should use manual calculation and resize
      const dimensions = manualManager.fit()

      // Verify terminal was resized with calculated dimensions
      expect(mockTerminal.resized).toBe(true)
      expect(dimensions).toBeDefined()
      expect(dimensions?.cols).toBeGreaterThan(0)
      expect(dimensions?.rows).toBeGreaterThan(0)
  ...
  describe('resize', () => {
    ...
    it('should handle resize when getCurrentDimensions returns null', async () => {
      // Context7 Best Practice: Test edge case where getCurrentDimensions returns null (line 530)
      await manager.initialize(
        mockTerminal as unknown as Terminal,
        mockContainer
      )
      ...
      // Mock getCurrentDimensions to return null after resize
      const originalGetCurrentDimensions = manager.getCurrentDimensions
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Mocking private method for testing
      ;(manager as any).getCurrentDimensions = vi.fn(() => null)

      manager.resize(90, 25)

      // Handler should not be called because dimensions is null
      expect(handler).not.toHaveBeenCalled()

      // Restore original method
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Restoring original method
      ;(manager as any).getCurrentDimensions = originalGetCurrentDimensions
    })
  })
  ...
  describe('ResizeObserver integration', () => {
    ...
    it('should ignore resize events from non-container targets', async () => {
      // Context7 Best Practice: Test ResizeObserver filtering logic (line 351)
      const autoManager = new TerminalResizeManager({
        autoResize: true,
        debounceDelay: 100,
      })
      ...
      observer.observe(differentElement)

      // Trigger resize on the wrong element
      observer.trigger(differentElement)

      // Fast-forward debounce timer
      vi.advanceTimersByTime(100)

      // Terminal should not have been resized (because entry.target !== this.container)
      expect(mockTerminal.resized).toBe(false)
  ...
  describe('onResize', () => {
    ...
    it('should handle errors in resize handlers', async () => {
      const debugManager = new TerminalResizeManager({ debug: true })
      await debugManager.initialize(
        mockTerminal as unknown as Terminal,
        mockContainer
    ...
    it('should handle errors in resize handlers without debug', async () => {
      // Context7 Best Practice: Test error handling without debug logging (line 499)
      const silentManager = new TerminalResizeManager({ debug: false })
      await silentManager.initialize(
        mockTerminal as unknown as Terminal,
        mockContainer
      )

      const errorHandler = vi.fn(() => {
        throw new Error('Silent handler error')
      })
      silentManager.onResize(errorHandler)
      ...
      // But console.error should not be called (because debug: false)
      expect(consoleErrorSpy).not.toHaveBeenCalled()

      silentManager.dispose()
    })
