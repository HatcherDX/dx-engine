...
describe('TerminalAddonManager', () => {
  ...
  describe('loadWebGLAddon (deprecated)', () => {
    it('should return null and log deprecation message', async () => {
    ...
    it('should log deprecation when debug is enabled', async () => {
      const consoleSpy = vi.spyOn(console, 'log')
      const debugManager = new TerminalAddonManager({ debug: true })
  ...
  describe('loadAllAddons', () => {
    it('should load all addons by default', async () => {
      ...
      expect(addons.webgl).toBeUndefined() // WebGL addon is deprecated
      ...
      expect(manager.getAddonCount()).toBe(5) // WebGL is not loaded
    })
  ...
  describe('debug mode disabled branches', () => {
    ...
    it('should not log when loading deprecated WebGL addon with debug disabled', async () => {
      const consoleSpy = vi.spyOn(console, 'log')
      const manager = new TerminalAddonManager({ debug: false })
    ...
    it('should not log when deprecated WebGL addon is called with debug disabled', async () => {
      const consoleSpy = vi.spyOn(console, 'log')
      const manager = new TerminalAddonManager({ debug: false })
