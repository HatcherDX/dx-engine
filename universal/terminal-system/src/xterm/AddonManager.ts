...
export class TerminalAddonManager {
   ...
   *
   * @param terminal - XTerm.js terminal instance
   * @returns Loaded WebglAddon instance or null if not supported
   *
   * @example
   * ```typescript
   * const webglAddon = await manager.loadWebGLAddon(terminal)
   * if (webglAddon) {
   *   console.log('WebGL acceleration enabled')
   * }
   * ```
   */
  async loadWebGLAddon(terminal: Terminal): Promise<WebglAddon | null> {
    try {
      // Check WebGL support first
      if (!this.isWebGLSupported()) {
        if (this.debug) {
          console.warn('[AddonManager] WebGL not supported, skipping addon')
        }
        return null
      }
      ...
      addon.onContextLoss(() => {
        if (this.debug) {
          console.warn('[AddonManager] WebGL context lost, disposing addon')
        }
        this.disposeAddon(AddonType.WEBGL)
      })

      terminal.loadAddon(addon)
      this.addons.set(AddonType.WEBGL, addon)

      if (this.debug) {
        console.log('[AddonManager] WebglAddon loaded successfully')
      }
  ...
  async loadAllAddons(
    ...
    if (options.enableWebGL !== false) {
      result.webgl = await this.loadWebGLAddon(terminal)
    }
