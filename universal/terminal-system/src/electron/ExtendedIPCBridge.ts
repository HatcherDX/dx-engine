...
export class ExtendedIPCBridge extends IPCBridge {
  ...
  private setupExtendedHandlers(): void {
    // Note: ipcMain is private in parent, need to access via any cast
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Accessing private ipcMain from parent IPCBridge class
    const ipcMain = (this as any).ipcMain as IpcMain
  ...
  > {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Terminal type from parent class not exported
    return terminals.map((term: any) => ({
      id: term.id,
      name: term.name || 'Terminal',
      isActive: true,
      stats: this.stats.get(term.id),
    }))
  }
