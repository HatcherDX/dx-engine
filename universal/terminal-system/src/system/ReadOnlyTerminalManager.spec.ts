/**
 * @fileoverview Comprehensive tests for ReadOnlyTerminalManager.
 *
 * @description
 * Tests for read-only terminal management system including:
 * - Terminal creation and initialization
 * - System and Timeline terminal management
 * - Event handling and routing from SystemLogger
...
import type { SystemEvent } from './SystemLogger'

/**
 * Mock interface for SystemLogger in readonly terminal manager tests.
 */
 ...
 */
interface ReadOnlyTerminalManagerInternal {
  formatTimestamp: (date: Date) => string
}

describe('ReadOnlyTerminalManager', () => {
  let manager: ReadOnlyTerminalManager
  ...
  beforeEach(() => {
    ...
    manager = new ReadOnlyTerminalManager(mockLogger)
  })
  ...
  describe('Constructor', () => {
    it('should create ReadOnlyTerminalManager instance', () => {
      expect(manager).toBeInstanceOf(ReadOnlyTerminalManager)
    })
    ...
    it('should initialize with provided logger', () => {
      ...
      const customManager = new ReadOnlyTerminalManager(customLogger)

      expect(customManager).toBeInstanceOf(ReadOnlyTerminalManager)
    })

    it('should use default logger when none provided', () => {
      const defaultManager = new ReadOnlyTerminalManager()

      expect(defaultManager).toBeInstanceOf(ReadOnlyTerminalManager)
    })
  ...
  describe('getActiveTerminal()', () => {
    ...
    it('should return active terminal', () => {
      ...
      expect(activeTerminal?.type).toBe('system')
      expect(activeTerminal?.isActive).toBe(true)
    })

    it('should return only one active terminal', () => {
      manager.setActiveTerminal('system')
      manager.setActiveTerminal('timeline')

      const activeTerminal = manager.getActiveTerminal()
      const allTerminals = manager.getAllTerminals()
      const activeCount = allTerminals.filter((t) => t.isActive).length

      expect(activeTerminal?.type).toBe('timeline')
  ...
  describe('getTerminalLines()', () => {
    ...
    it('should filter lines by date', async () => {
      // Add initial lines
      manager.addLine('system', 'Old message 1', 'INFO')
      manager.addLine('system', 'Old message 2', 'INFO')

      // Wait a moment to ensure time difference
      await new Promise((resolve) => setTimeout(resolve, 10))
      const cutoffTime = new Date()

      // Wait another moment before adding new lines
      await new Promise((resolve) => setTimeout(resolve, 10))
  ...
  describe('Timestamp Formatting', () => {
    it('should format timestamps correctly', () => {
      ...
      const formatted = (
        manager as unknown as ReadOnlyTerminalManagerInternal
  ...
  describe('Integration with Default Instance', () => {
    it('should provide readOnlyTerminalManager default instance', () => {
      expect(readOnlyTerminalManager).toBeInstanceOf(ReadOnlyTerminalManager)
    })

    it('should be separate from manually created instances', async () => {
      const manualManager = new ReadOnlyTerminalManager(mockLogger)

      await readOnlyTerminalManager.initializeSystemTerminals()
      await manualManager.initializeSystemTerminals()

      expect(readOnlyTerminalManager.getAllTerminals()).toHaveLength(2)
      expect(manualManager.getAllTerminals()).toHaveLength(2)

      // They should be separate instances
      expect(readOnlyTerminalManager.getTerminal('system')).not.toBe(
