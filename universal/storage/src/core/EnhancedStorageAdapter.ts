  ...
  implements IStorageAdapter
{
  ...
  private async runWithMiddleware<T>(
    ...
    const chain = this.middlewares.reduceRight(
      (next, middleware) => () => middleware.process(context, next),
      handler
    )
  ...
  private recordOperationSuccess(): void {
    ...
    for (const strategy of this.errorStrategies) {
      if (
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Strategy interface doesn't expose recordSuccess, accessing dynamic method
        ;(strategy as any).recordSuccess()
      }
    }
  }
  ...
  protected async closeEnhanced(): Promise<void> {
    ...
    if (this.performanceMonitor && 'dispose' in this.performanceMonitor) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- PerformanceMonitor interface doesn't expose dispose, accessing dynamic method
