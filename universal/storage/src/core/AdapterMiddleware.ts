...
export interface AdapterMiddleware {
   ...
   */
  process(context: MiddlewareContext, next: MiddlewareNext): Promise<unknown>
}
...
export class LoggingMiddleware implements AdapterMiddleware {
  ...
  private formatStartMessage(operation: string, args: unknown[]): string {
    ...
    const message =
      error instanceof Error
        ...
        : typeof error === 'string'
          ? error
          : String(error)
    return `[Storage] ${operation} failed after ${duration}ms: ${message}`
  }
...
export class ValidationMiddleware implements AdapterMiddleware {
  ...
  private validateKeys(keys: unknown): void {
  ...
  private validatePrefix(prefix: unknown): void {
  ...
  private validateBatch(batch: unknown): void {
...
export class MetricsMiddleware implements AdapterMiddleware {
  ...
  private recordMetrics(
    ...
    const metrics: PerformanceMetrics & Record<string, unknown> = {
      operation,
      duration,
      success,
