...
export interface ServiceProvider<T> {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Generic config parameter accepts any service configuration
...
export class MockServiceProvider<T> implements ServiceProvider<T> {
   ...
   *
   * @param _config - Configuration (ignored for mocks)
   * @returns Promise that resolves to the mock service
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unused-vars -- Generic config parameter accepts any type; parameter unused in mock implementation
  async create(_config?: any): Promise<T> {
