  ...
  try {
    ...
    if (window.storageAPI) {
      try {
        ...
        // CRITICAL: Only skip onboarding if workspace has a valid project with non-empty path
        if (
          workspace &&
          workspace.project &&
          workspace.project.path &&
          workspace.project.path.trim() !== ''
        ) {
          console.log(
            '[Onboarding] Existing workspace with valid project found, keeping IDE view'
          )
          ...
          console.log(
            '[Onboarding] No valid workspace found, activating onboarding'
          )
          // No workspace or empty project path, activate onboarding
          state.value = {
            isFirstTime: true,
            currentStep: 'welcome',
            selectedTask: null,
