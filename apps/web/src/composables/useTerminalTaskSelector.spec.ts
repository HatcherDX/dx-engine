...
describe('useTerminalTaskSelector', () => {
  ...
  describe('updateMessages', () => {
    ...
    it('should handle loading state with animation', async () => {
      ...
      setTimeoutSpy.mockImplementation(
        // eslint-disable-next-line @typescript-eslint/no-unused-vars -- Mock function signature for testing
  ...
  describe('🎯 Coverage: Keyboard handling in search mode (lines 425-539)', () => {
    it('should handle Escape key in search mode', async () => {
      ...
      // Set up search mode and provide some terminal content
      mockCurrentContext.state.lines = [{ text: 'Line 1' }, { text: 'Line 2' }]
      ...
      // Handle the key event with all required parameters
      selector.handleKeyPress(
        escapeEvent,
        searchQuery,
        branches,
      ...
      // Verify that no errors occurred during the key handling
    ...
    it('should handle Enter key in search mode', async () => {
      ...
      selector.handleKeyPress(enterEvent)

      // Since this is testing keyboard handling, verify no errors occurred
      expect(selector.isTerminalSearchMode.value).toBe(true)
