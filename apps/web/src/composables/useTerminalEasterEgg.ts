...
export function useTerminalEasterEgg() {
  ...
  function handleCommandFromMain(command: string): void {
    ...
    if (window.isTerminalSearchMode) {
  ...
  function handleInputFromMain(char: string): void {
    ...
    if (char.toLowerCase() === 'b' && currentStep.value === 'task-selector') {
      ...
      const searchModeEnteredTime = window.searchModeEnteredTime || 0
  ...
  onMounted(() => {
    ...
    handleDeactivateFromUI = () => {
      ...
      window.__terminalDeactivatedViaUI = true
      deactivateTerminal()
    ...
    handleClearSearch = (event: CustomEvent) => {
      if (event.detail?.step === 'branch-creation') {
        ...
        window.terminalBranchSearchQuery = ''
    ...
    if (window._terminalListenersRegistered) {
      console.log(
      ...
      window._terminalListenersRegistered = true
      ...
      window.electronAPI.terminalEasterEgg.onInput((data) => {
        ...
        if (!window._receivedMessages) {
          window._receivedMessages = new Set()
        }

        // Use sendId from data if provided (for testing), otherwise generate timestamp-based ID
        const messageId =
          (data as { sendId?: string; char: string; step: string }).sendId ||
          `${data.char}-${data.step}-${Date.now()}`

        if (window._receivedMessages.has(messageId)) {
          console.warn(
        ...
        window._receivedMessages.add(messageId)

        // Clean up old entries (keep last 100)
        if (window._receivedMessages.size > 100) {
          const entries = Array.from(window._receivedMessages)
          entries.slice(0, entries.length - 100).forEach((id) => {
            window._receivedMessages?.delete(id)
          })
        }
