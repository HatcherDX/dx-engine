...
import type { TerminalCommand, OnboardingAPI } from './terminalContextFactory'
...
export interface TerminalStrategy {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Terminal context requires flexible typing for cross-context message coordination
  handleCharacterInput?: (char: string, context: any) => boolean
  /** Custom behavior for command execution */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Terminal context requires flexible typing for cross-context message coordination
  preCommandExecution?: (command: string, context: any) => boolean
}
...
export class TerminalStrategies {
  ...
  private static getWelcomeStrategy(
    ...
    return {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Terminal context requires flexible typing for cross-context message coordination
      handleCharacterInput: (char: string, context: any) => {
    ...
    return {
      ...
      handleCharacterInput: (char: string, context: any) => {
        ...
        if (
          ...
          const currentInput =
            context?._state?.currentInput || context?.state?.currentInput || ''
  ...
  private static getTaskDetailStrategy(
    ...
    return {
      ...
      handleCharacterInput: (char: string, context: any) => {
        ...
        const bridge = (window as any).terminalInputBridge
        if (!bridge) {
        ...
        if (handled) {
          ...
          context._state.cursorPosition = newCursor
  ...
  private static getBranchCreationStrategy(
    ...
    const getBranchInfo = () => {
      const storedBranch = onboardingAPI.getSelectedBranch?.value || {}
    ...
    const getAvailableBranches = () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Runtime dynamic typing required for flexible data handling
      const storedBranches = (window as any).terminalAvailableBranches
      if (
        storedBranches &&
      ...
      if (branchSelectorEl) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Runtime dynamic typing required for flexible data handling
        const vueInstance = (branchSelectorEl as any).__vueParentComponent
        if (
          vueInstance &&
          vueInstance.props &&
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Runtime dynamic typing required for flexible data handling
          ;(window as any).terminalAvailableBranches =
            vueInstance.props.branches
    ...
    const selectBranch = (branchName: string) => {
      ...
      if (selector) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Runtime dynamic typing required for flexible data handling
        const vueInstance = (selector as any).__vueParentComponent
        if (vueInstance) {
    ...
    return {
      ...
      handleCharacterInput: (char: string, context: any) => {
        if (isInSearchMode()) {
          ...
          if (context) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Strategy pattern requires flexible typing for dynamic context injection
