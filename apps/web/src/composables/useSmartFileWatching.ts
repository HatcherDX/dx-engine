...
export function useSmartFileWatching() {
  ...
  const startSmartWatching = async (projectPath: string): Promise<void> => {
    ...
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Function extension stores cleanup handler for lifecycle management
      ;(startSmartWatching as any)._cleanup = cleanup
    } catch (error) {
      console.error(
  ...
  const stopSmartWatching = async (): Promise<void> => {
    ...
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Function extension retrieves cleanup handler for lifecycle management
      const cleanup = (startSmartWatching as any)._cleanup
      if (typeof cleanup === 'function') {
        cleanup()
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Function extension cleanup removal after execution
        delete (startSmartWatching as any)._cleanup
      }
