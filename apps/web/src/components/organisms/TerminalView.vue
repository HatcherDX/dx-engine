...
// eslint-disable-next-line no-undef -- NodeJS global type available in TypeScript
    ...
    terminalInstance.value = await XTerminalFactory.createTerminal(
      terminalContainer.value,
      {
        ...
        terminal: {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Terminal library requires flexible theme type
          theme: terminalTheme.value as any,
...
watch(
  ...
  () => {
    if (terminalInstance.value) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Terminal library requires flexible theme type
      terminalInstance.value.terminal.options.theme = terminalTheme.value as any
    }
  }
)
