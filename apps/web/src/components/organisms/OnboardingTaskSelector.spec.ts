...
describe('OnboardingTaskSelector.vue', () => {
  ...
  describe('📊 Coverage Edge Cases - 100% Target', () => {
    it('should handle highlightMatch with empty text parameter', () => {
      const wrapper = mount(OnboardingTaskSelector)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Test component instance requires type assertion for internal method access
      const vm = wrapper.vm as any
    ...
    it('should handle selectExistingBranch with successful switch but null selectedBranch', async () => {
      // Mock selectBranchFromManager to CLEAR selectedBranch (edge case)
      mockSelectBranchFromManager.mockImplementation(async () => {
        // Clear selectedBranch.value, simulating composable handling the switch differently
        mockSelectedBranch.value = null
      })
      ...
      // Should log "Branch switch handled by composable" message
      expect(mockSelectBranchFromManager).toHaveBeenCalled()
      expect(wrapper.emitted('next')).toBeFalsy()
    })

    it('should not activate search mode when already in search mode', async () => {
      const wrapper = mount(OnboardingTaskSelector)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Test component instance requires type assertion for internal method access
      const vm = wrapper.vm as any

      // Set both flags to true to hit the guard clause
      vm.isInSearchMode = true
      mockIsTerminalSearchMode.value = true

      vm.activateSearchMode()

      // Should not enter search mode again
      expect(vm.isInSearchMode).toBe(true)
    })

    it('should keep search mode active in handleSearchBlur when query exists', async () => {
      vi.useFakeTimers()
      const wrapper = mount(OnboardingTaskSelector)
      const searchInput = wrapper.find('.search-input')
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Test component instance requires type assertion for internal method access
      ...
      // Should keep search mode active when query exists
      expect(vm.isInSearchMode).toBe(true)
      vi.useRealTimers()
    })

    it('should not preventDefault for keys not in handled list when in terminal mode', async () => {
      mockIsTerminalSearchMode.value = true
      const wrapper = mount(OnboardingTaskSelector)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Test component instance requires type assertion for internal method access
      const vm = wrapper.vm as any

      // Test a multi-character key that's not in handledKeys
      const event = new KeyboardEvent('keydown', { key: 'Tab' })
      const preventDefaultSpy = vi.spyOn(event, 'preventDefault')

      vm.handleKeyPress(event)

      // Should NOT prevent default for Tab (multi-char key not in handled list)
      expect(preventDefaultSpy).not.toHaveBeenCalled()
    })

    it('should handle activateSearchMode when isTerminalSearchMode is true', async () => {
      mockIsTerminalSearchMode.value = true
      const wrapper = mount(OnboardingTaskSelector)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Test component instance requires type assertion for internal method access
      const vm = wrapper.vm as any
      ...
      // Should not activate when terminal search mode is active
      expect(vm.isInSearchMode).toBe(false)
    })

    it('should handle loadBranchesWithTerminal when window.currentContext is false', async () => {
      window.currentContext = false
      mockLoadBranches.mockClear()
      mockUpdateTerminalMessages.mockClear()
      ...
      expect(mockUpdateTerminalMessages).not.toHaveBeenCalled()

      // Restore
      window.currentContext = true
    })
  })
})
