  ...
  async (
    ...
    try {
      ...
      const watcher = chokidar.watch(projectPath, {
        // Ignore patterns to avoid noise and reduce file descriptors
        ignored: [
          '**/node_modules/**',
          '**/dist/**',
          '**/build/**',
      ...
      activeWatchers.set(watcherId, watcherWithCleanup as any)
...
electronIpcMain.handle(
  ...
  async (
    ...
    try {
      ...
      // Call cleanup function if it exists (for new watchers with coalescence)
      if (typeof watcherData === 'object' && watcherData.cleanup) {
        watcherData.cleanup()
        watcherData.watcher.close()
      } else {
        // Legacy watcher without cleanup
        watcherData.close()
      }
